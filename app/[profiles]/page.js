...
const Lotiecontroler = dynamic(() => import('@/components/Lotiecontroler'), { ssr: false });
  ...
  return (
    <main className='bg-gradient-to-b from-gray-100 to-blue-100  '>
      ...
      <div className='bg-gradient-to-b from-gray-100 to-blue-100 pt-7 mt-8 text-black md:w-[60%] w-[80%] min-h-screen rounded-3xl fixed top-10 md:left-[11%] left-[10%] px-3 pb-10 '>
        ...
        <div className={`${showAdd ? "" : "hidden"} fixed md:right-[50vw] m-auto top-40 flex justify-center items-center z-1  `} id='addlinks_modal'>
        ...
        {displayLinks && displayLinks.map((link, index) => {
          return <section className='bg-cyan-100 md:m-1.5 w-[90%] mt-2 md:mt-4 m-0.5 p-1.5 border-2 border-blue-700 border-dotted rounded-2xl shadow-md drop-shadow-xl hover:shadow-[#01065a] ' key={link._id}>
            <div id="link" className=' md:pl-7 pl-3 md:gap-25 items-center flex  md:flex-row justify-between  '>
              ...
              <div className='buttons '>
                <label className="inline-flex items-center md:me-5 cursor-pointer">
                ...
                <Lotiecontroler src="/delete.lottie" label="" className="cursor-pointer relative top-1 pl-0.5 " cl="w-10 h-10" onClick={() => { handleDelete(link._id) }} />
              </div>
            </div>
          </section >
        })}
      ...
      <div className='fixed top-17 md:block hidden right-10 w-1/4 h-5/6 overflow-hidden rounded-3xl border-4 border-blue-900 '>
        <Preview heightHg={"100vh"} widthHg={"30vw"} changer={getChanger}/>
      </div>
    </main>
  )
}
